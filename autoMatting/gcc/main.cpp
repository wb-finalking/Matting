...
cv::Mat getAlpha(double lambda, int win_size, double epsilon, cv::Mat input, cv::Mat consts_map, cv::Mat consts_vals)
{
	...
	for (int i = 0; i < img_size; i++) {
		D.coeffRef(i, i) = (int)consts_map_trans.at<double>(0, i);
	}
...
int main()
{
	...
	cv::Mat img_m = cv::imread("./trimap.png");
	img.convertTo(img,CV_64FC3);
	img_m.convertTo(img_m,CV_64FC3);
	std::vector<cv::Mat> input_m_channels(3);
	split(img_m,input_m_channels);
	
	cv::Mat alpha = performMatting(lambda, win_size, epsilon, img, input_m_channels[0]);
	
	//img_m.convertTo(img_m,CV_64FC1);
