...
SpMat getLaplacianMatrix<CPUDevice>(const CPUDevice& d, int win_size, double epsilon, cv::Mat input)
{
	int len = 0;
	//int win_size = 3;
	cv::Mat repe_col;
...
class MattingOp : public OpKernel {
	...
	void Compute(tensorflow::OpKernelContext* context) override
	{
		...
		{
			cv::Mat input(cv::Size(input_cols,input_rows),CV_64FC3,const_cast<T*>(I_)+step*i*3);
			cv::Mat input_fb(cv::Size(input_cols,input_rows),CV_64FC3,const_cast<T*>(FB_)+step*i*3);
	...
	*/
	cv::Mat getAlpha(T lambda, int win_size, double epsilon, cv::Mat input, cv::Mat input_f, cv::Mat input_b)
	{
		...
		SpMat A = getLaplacianMatrix<Device>(context->eigen_device<Device>(), win_size, epsilon, input);
...
#define REGISTER_GPU(T)                                          \
  ...
  extern template SpMat getLaplacianMatrix<GPUDevice>(const GPUDevice& d, int win_size, double epsilon, cv::Mat input);             \
  REGISTER_KERNEL_BUILDER(                                       \
      Name("Matting").Device(DEVICE_GPU).TypeConstraint<T>("T"), \
      MattingOp<GPUDevice, T>);
REGISTER_GPU(double);
#endif  // GOOGLE_CUDA
...
class MattingGradOp : public OpKernel {
	...
	void Compute(tensorflow::OpKernelContext* context) override
	{
		...
		for(int i=0;i<batch;i++)
		{
			...
			cv::Mat input_fb(cv::Size(input_cols,input_rows),CV_64FC3,const_cast<T*>(FB_)+step*i*3);
			//cv::Mat input_b(cv::Size(input_cols,input_rows),CV_64FC1,const_cast<T*>(B_)+step*i);
			cv::Mat grad(cv::Size(input_cols,input_rows),CV_64FC1,const_cast<T*>(Grad_in_)+step*i);
			T lambda =Lambda_(0);
	...
	void getGrad(cv::Mat input, cv::Mat input_f, cv::Mat input_b, T lambda, cv::Mat grad, cv::Mat& F_grad, cv::Mat& B_grad, T& Lamb_grad)
	{
		...
		SpMat Laplace = getLaplacianMatrix<Device>(context->eigen_device<Device>(), 1, 0.00001, input);
		...
		SpMat D = lambda * F_diag + lambda * F_diag + Laplace;

		SpMat Eye(img_size, img_size);
		for (int i = 0; i < img_size; i++) {
			Eye.coeffRef(i, i) = 1;
		}
...
#define REGISTER_GPU(T)                                          \
  ...
  extern template SpMat getLaplacianMatrix<GPUDevice>(const GPUDevice& d, int win_size, double epsilon, cv::Mat input);             \
  REGISTER_KERNEL_BUILDER(                                       \
      Name("MattingGrad").Device(DEVICE_GPU).TypeConstraint<T>("T"), \
      MattingGradOp<GPUDevice, T>);
REGISTER_GPU(double);
#endif  // GOOGLE_CUDA
